...
template <int N, int M, int block_size, int group2BatchSize,
          int numGroup2BatchesPerBlock>
__global__ void computeCoordinationNumberTwoGroupsCUDAKernel2(
  const bool* __restrict d_use_pairlist,
  const bool* __restrict d_rebuild_pairlist,
  const double* __restrict pos1x,
  const double* __restrict pos1y,
  const double* __restrict pos1z,
  const double* __restrict pos2x,
  ...
  unsigned int* __restrict tbcount,
  double* __restrict energy_out) {
  const bool use_pairlist = *d_use_pairlist;
  const bool rebuild_pairlist = *d_rebuild_pairlist;
  // TODO: Figure out a way to remove this limitation
  static_assert(block_size == group2BatchSize * numGroup2BatchesPerBlock,
                "block_size != group2BatchSize * numGroup2BatchesPerBlock");
#if defined (USE_CG_SYNC)
      ...
      for (unsigned int t = 0; t < group2BatchSize; ++t) {
        ...
        if (mask_i && mask_jid) {
          ...
          if (!(use_pairlist)) {
            ...
            if (!(rebuild_pairlist)) {
              coordnum_pairlist<N, M, true, false>(
                x1, x2, y1, y2, z1, z2, inv_r0, inv_r0, inv_r0, ei,
                iGrad.x, iGrad.y, iGrad.z,
                shJGrad[group2BatchID][jid].x,
                shJGrad[group2BatchID][jid].y,
              ...
              bool p;
              coordnum_pairlist<N, M, true, true>(
                x1, x2, y1, y2, z1, z2, inv_r0, inv_r0, inv_r0, ei,
                iGrad.x, iGrad.y, iGrad.z,
                shJGrad[group2BatchID][jid].x,
                shJGrad[group2BatchID][jid].y,
...
void ComputeCoordinationNumberTwoGroups2::initialize(
  unsigned int numAtoms1, unsigned int numAtoms2,
  bool enablePairlist, double pairlistTolerance) {
  m_numAtoms1 = numAtoms1;
  m_numAtoms2 = numAtoms2;
  if (initialized) {
    if (d_tbcount) checkGPUError(cudaFree(d_tbcount));
    if (d_pairlist) checkGPUError(cudaFree(d_pairlist));
    if (d_energy_tmp) checkGPUError(cudaFree(d_energy_tmp));
    if (d_use_pairlist) checkGPUError(cudaFree(d_use_pairlist));
    if (d_rebuild_pairlist) checkGPUError(cudaFree(d_rebuild_pairlist));
  }
  m_enablePairlist = enablePairlist;
  if (enablePairlist) {
    pairlistSize = m_numAtoms1 * m_numAtoms2;
    m_pairlistTolerance = pairlistTolerance;
    pairlistTransposed = m_numAtoms1 > m_numAtoms2;
  ...
  checkGPUError(cudaMalloc(&d_tbcount, sizeof(unsigned int)));
  checkGPUError(cudaMemset(d_tbcount, 0, sizeof(unsigned int)));
  checkGPUError(cudaMalloc(&d_use_pairlist, sizeof(bool)));
  checkGPUError(cudaMemset(d_use_pairlist, enablePairlist, sizeof(bool)));
  checkGPUError(cudaMalloc(&d_rebuild_pairlist, sizeof(bool)));
  checkGPUError(cudaMemset(d_rebuild_pairlist, enablePairlist, sizeof(bool)));
  initialized = true;
}

ComputeCoordinationNumberTwoGroups2::~ComputeCoordinationNumberTwoGroups2() {
  if (d_tbcount) checkGPUError(cudaFree(d_tbcount));
  if (d_pairlist) checkGPUError(cudaFree(d_pairlist));
  if (d_energy_tmp) checkGPUError(cudaFree(d_energy_tmp));
  if (d_use_pairlist) checkGPUError(cudaFree(d_use_pairlist));
  ...
  d_pairlist = nullptr;
  d_energy_tmp = nullptr;
  d_use_pairlist = nullptr;
  d_rebuild_pairlist = nullptr;
}

std::vector<char>ComputeCoordinationNumberTwoGroups2::pairlistToHost() const {
  std::vector<char> out(pairlistSize);
  if (pairlistSize > 0) checkGPUError(cudaMemcpy(
    out.data(), d_pairlist, sizeof(char) * pairlistSize, cudaMemcpyDeviceToHost));
  if (pairlistSize > 0 && pairlistTransposed) {
...
void ComputeCoordinationNumberTwoGroups2::addComputeToGraph(
  const AtomGroupPositionsCUDA& group1,
  const AtomGroupPositionsCUDA& group2,
  AtomGroupGradientsCUDA& gradient1,
  AtomGroupGradientsCUDA& gradient2,
  ...
  if (numAtoms2 > numAtoms1) {
    addComputeToGraph(
      group2, group1, gradient2, gradient1, inv_r0,
      energy_out, node, dependencies, graph);
    return;
  }
  const double* pos1x = group1.getDeviceX();
  const double* pos1y = group1.getDeviceY();
  ...
  cudaKernelNodeParams kernelNodeParams = {0};
  kernelNodeParams.blockDim       = dim3(default_block_size, 1, 1);
  kernelNodeParams.sharedMemBytes = m_enablePairlist ? default_block_size * default_block_size * sizeof(bool) : 0;
  kernelNodeParams.kernelParams   = args;
  kernelNodeParams.extra          = NULL;
#define FUNC_CALL(GROUP2WORKSIZE) \
  do {\
    kernelNodeParams.func = (void*)computeCoordinationNumberTwoGroupsCUDAKernel2<6, 12, default_block_size, GROUP2WORKSIZE, default_block_size / GROUP2WORKSIZE>;\
  } while (0)
  if (minNumAtoms > 128) {
    FUNC_CALL(128);
  } else if (minNumAtoms > 64) {
...
void ComputeCoordinationNumberTwoGroups2::addUpdateFlagsToGraph(
  ...
  const std::vector<cudaGraphNode_t>& dependencies,
  cudaGraph_t& graph) {
  cudaMemsetParams param{0};
  param.elementSize = 1;
  param.value = use_pairlist_in ? 1 : 0;
  param.width = sizeof(bool);
  param.height = 1;
  param.dst = d_use_pairlist;
  checkGPUError(cudaGraphAddMemsetNode(&node_use_pairlist, graph, dependencies.data(), dependencies.size(), &param));
  param.value = rebuild_pairlist_in ? 1 : 0;
  param.dst = d_rebuild_pairlist;
  checkGPUError(cudaGraphAddMemsetNode(&node_rebuild_pairlist, graph, dependencies.data(), dependencies.size(), &param));
}
...
void ComputeCoordinationNumberSelfGroupCUDA::prepareTilesList(unsigned int numAtoms) {
  const unsigned int numTiles = (numAtoms + tileSize - 1) / tileSize;
...
ComputeCoordinationNumberSelfGroupCUDA::ComputeCoordinationNumberSelfGroupCUDA(unsigned int self_group_block_size_in, unsigned int tile_size_in) {
  selfGroupBlockSize = self_group_block_size_in;
  tileSize = tile_size_in;
  auto it = std::find(
    std::cbegin(selfGroupBlockSizeSupported),
    std::cend(selfGroupBlockSizeSupported),
    selfGroupBlockSize);
  if (it == std::cend(selfGroupBlockSizeSupported)) {
    const std::string error = fmt::format("Unsupported block size: {}\n", selfGroupBlockSize);
    throw error;
  }
  it = std::find(
    std::cbegin(tileSizeSupported),
    std::cend(tileSizeSupported),
    tileSize);
  if (it == std::cend(tileSizeSupported)) {
    const std::string error = fmt::format("Unsupported tile size: {}\n", tileSize);
    throw error;
  }
}
...
void ComputeCoordinationNumberSelfGroupCUDA::addComputeToGraph(
  ...
  void* args[] = {
    ...
    &energy_out};
  cudaKernelNodeParams kernelNodeParams = {0};
  kernelNodeParams.blockDim       = dim3(getBlockSize(), 1, 1);
  kernelNodeParams.sharedMemBytes = m_usePairlist ?
    getBlockSize() * sizeof(unsigned int) : 0;
  kernelNodeParams.kernelParams   = args;
