...
void __host__ __device__ coordnum(
  double x1, double x2,
  double y1, double y2,
  double z1, double z2,
  double inv_r0,
  ...
  const double dx = (x2 - x1) * inv_r0;
  const double dy = (y2 - y1) * inv_r0;
  const double dz = (z2 - z1) * inv_r0;
  const double r2 = dx * dx + dy * dy + dz * dz;
  int const en2 = N/2;
  int const ed2 = M/2;
  double const xn = integer_power(r2, en2);
  double const xd = integer_power(r2, ed2);
  double const func = (1.0-xn)/(1.0-xd);
  ...
  if (func > 0.0) {
    ...
    const double dr2_dx = 2.0 * dx * inv_r0;
    const double dr2_dy = 2.0 * dy * inv_r0;
    const double dr2_dz = 2.0 * dz * inv_r0;
